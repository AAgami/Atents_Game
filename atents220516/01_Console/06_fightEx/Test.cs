...
namespace _06_fightEx
{
    class Test
    {
        ...
        public void Test4_HumanVSOrc()
        {
            ...
            Orc orc = new Orc(20, 20, 1, "1번 오크");
            //논리 연산자
            //&&(and) - 양쪽 값이 참이면 참, 아니면 거짓
            //|| (or) - 양쪽 값이 하나라도 참이면 참.
            

            while ((human.HealthPoint > 0) && (orc.HealthPoint > 0)) //둘중 한명이라도 hp가 0보다 작거나 같아지면 종료
            {
                human.Attack(orc);
                orc.Attack(human);
            }
        ...
        {
            Human human1 = new Human(20, 10, 10, "Player 1");
            Human human2 = new Human(15, 15, 15, "Player 2");
            Orc orc1 = new Orc(20, 20, 20,"name");

            human1.HealthPoint = 50;
            Console.WriteLine($"Human1 health:{human1.HealthPoint}");

            human2.HealthPoint = 100;
            Console.WriteLine($"Human2 health:{human2.HealthPoint}");

            orc1.HealthPoint = 100;
            Console.WriteLine($"orc1 health:{orc1.HealthPoint}");
        ...
        public void Test2_ClassTest()
        {
            ...
            humanInstance = new Human(10,10,10); //인스턴스를 만들었다
            ...
            //Human이 힐을 받아서 Healthpoint가 50 상승했다.

            Console.WriteLine($"Human이 힘을 받아서 HealPoint가 50 상승했다.");
            humanInstance.HealthPoint += 50;
            Console.WriteLine($"Health Point:{humanInstance.HealthPoint}");

            // hp = 140. humanInstance.HealthPoint = humanInstance.HealthPoint + 50;           
            ...
            //Human이 공격을 당해서 HealthPoint가 200 감소했다. 


            Console.WriteLine($"Human이 공격을 당해서 HealthPoint가 200 감소했다.");
        ...
        public void Test1_VariableFunctionContol()
        {
            ...
            PrintStatus(userStr, userDex, userInt,100); // PrintStatus 함수 호출
        ...
        void PrintStatus(int str, int dex, int intelligence, int health)
        {
            Console.WriteLine($"┌ 스테이터스 ────────────────────────────┐");
            Console.WriteLine($"│ 체력 : {health,5}                           │");
